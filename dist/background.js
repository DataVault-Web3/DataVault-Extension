...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  if (message.type === 'orderDataExtracted') {
    ...
    var processedData = processOrderDataWithCrypto(message.data);
  ...
  if (message.type === 'getProcessedData') {
    chrome.storage.local.get(['processedOrders'], function (result) {
      sendResponse({
        data: result.processedOrders
      });
    });
...
function processOrderDataWithCrypto(data) {
  try {
    ...
    var stableStringified = json_stable_stringify_default()(data);
    ...
    var signature = keccak256(toUtf8Bytes(stableStringified + Date.now()));
    return {
      originalData: data,
      stableHash: hash,
      signature: signature,
